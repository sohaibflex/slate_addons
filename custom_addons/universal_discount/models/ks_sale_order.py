...
class KsGlobalDiscountSales(models.Model):
    ...
    ks_untaxed_amount_after_discount = fields.Monetary(string='Untaxed Amount After Discount', readonly=True, compute='_get_ks_untaxed_amount_after_discount', store=True,
                                         track_visibility='always')
    ks_enable_discount = fields.Boolean(compute='ks_verify_discount')

    @api.depends('order_line.price_total', 'ks_global_discount_rate', 'ks_global_discount_type')
    def _get_ks_untaxed_amount_after_discount(self):
        for rec in self:
            rec.ks_untaxed_amount_after_discount = rec.amount_untaxed - rec.ks_amount_discount
    ...
    def ks_calculate_discount(self):
        for rec in self:
            ...
            if rec.amount_untaxed != 0:
                tax_percentage = amount_tax / rec.amount_untaxed
            else:
                tax_percentage = 0
            # rec.ks_untaxed_amount_after_discount = rec.amount_untaxed - rec.ks_amount_discount
            rec.amount_tax = tax_percentage * rec.ks_untaxed_amount_after_discount
            rec.amount_total = rec.ks_untaxed_amount_after_discount + rec.amount_tax
